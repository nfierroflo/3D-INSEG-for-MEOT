...
%Poisson intensity
birthmodel = struct('w',log(0.1),'xr',[],'Cr',diag(6.66*[50 50 1 1])^2,'V',[],'v',100,'alpha',500,'beta',100);
%specify kinematic state (x-position,y-position,x-velocity,y_velocity)
birthmodel.xr = [0 0 0 0]';
%specify extent state (orientation,two axis lengths)
birthmodel.V = (100+6)*diag([0.5 0.5]);
%Poisson birth rate
b_lambda_0 = 5;
...
person1_gt=readtable("data\2021-12-10-06-52-32\groundtruth\person1.csv");
person2_gt=readtable("data\2021-12-10-06-52-32\groundtruth\person2.csv");
gt = struct('x_bt',[],'x_dt',[],'x_lambda',[],'xr',[],'X',[]);
gt(1).x_bt=1;
duration=length(person1_gt.Center_X);
gt(1).x_dt=duration;
gt(1).x_lambda=10;
gt(1).xr=[person1_gt.Center_X person1_gt.Center_Y zeros(duration,1) zeros(duration,1)]';
gt(1).X=repmat(diag([0.25 0.25]),[1,1,duration]);

gt(2).x_bt=1;
duration=length(person2_gt.Center_X);
gt(2).x_dt=duration;
gt(2).x_lambda=10;
gt(2).xr=[person2_gt.Center_X person2_gt.Center_Y zeros(duration,1) zeros(duration,1)]';
gt(2).X=repmat(diag([0.25 0.25]),[1,1,duration]);
...
load("data\crossing_paths.mat")
extended_measurements = cell(T,1);

for i=1:T
    concat=cat(2,person1.Zextend{i}(1:10,:)',person2.Zextend{i}(1:10,:)');
    extended_measurements{i}=concat;
end

Z=extended_measurements;
...
for t = 1:T
    ...
    gating_matrix_u(:) = 1;
    gating_matrix_s(:) = 0;
    for i=1:length(gating_matrix_d)
        gating_matrix_d{i}(:) = 1;
    end
    ...
    cluster_1=[ones(10,1);zeros(10,1)];
    cluster_2=[zeros(10,1);ones(10,1)];
    partitions{1}=logical([cluster_1,cluster_2]);
%     %number of partitions
    ...
    for i = 1:m
        ...
        mbm_upd.l{n_track+i,1} = tracks_new{i}.l;
...
colors=['b','r','g','y'];
hold on;
for k=1:1:size(est,1)
    plot(est{k}.xr(1,:),est{k}.xr(2,:),'+')
    
    for i=1:est{k}.card
        text(est{k}.xr(1,i),est{k}.xr(2,i),"label:"+est{k}.l(1,i)+","+est{k}.l(2,i))
        plot_extent_iw(est{k}.xr(1:2,i),est{k}.X(:,:,i),'-',colors(i),1);
    end
end
title("Estimation")
...
hold on;
for k=1:1:size(Z,1)
    plot(Z{k}(1,1:10),Z{k}(2,1:10),'r+')
    plot(Z{k}(1,10:20),Z{k}(2,10:20),'b+')
end
title("Measurements")
...
hold on;
for i=1:size(gt,2)
    for k=1:1:size(gt(i).xr,2)
        plot(gt(i).xr(1,:),gt(i).xr(2,:),'+')
        plot_extent_iw(gt(i).xr(1:2,k),gt(i).X(:,:,k),'-',colors(i),1);
